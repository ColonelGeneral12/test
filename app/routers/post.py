...
def get_posts(db: Session = Depends(get_db), current_user: int = Depends(oath2.get_current_user), limit: int = 6,
...
def delete_post(id: int, db: Session = Depends(get_db), current_user: int = Depends(oath2.get_current_user)):
    print(current_user.id)  # Current_user is a memory location.
    ...
    if post.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail=f"Not authorized to perform the given action.")
...
@router.put("/{id}", response_model=schemas.Post,
            status_code=status.HTTP_200_OK)  # explicitly defining the http response code is a Must in odrder to avoid authentication errors
